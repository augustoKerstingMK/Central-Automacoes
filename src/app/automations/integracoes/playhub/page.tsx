...
export default function PlayhubSyncPage() {
  ...
  useEffect(() => {
    ...
    if (!storedToken || !storedDomain) {
      ...
      addLog("info", `Sessão iniciada p/ PLAYHUB no domínio: ${storedDomain}`);
    }
  }, [router]);
  ...
  const processQueue = async () => {
    ...
    addLog("info", "Iniciando integração de contratos PLAYHUB em lote...");
    ...
    for (let i = 0; i < tasks.length; i++) {
      ...
      try {
        const res = await fetch("/api/mk/playhub", {
          method: "POST",
                    ...
                    <Table className="w-full border-b-0">
                      <TableHeader className="bg-white sticky top-0 z-10 shadow-[0_2px_4px_rgba(0,0,0,0.02)]">
                        <TableRow className="hover:bg-transparent border-gray-200">
                          ...
                          <TableHead className="font-semibold text-[#414040] py-4 px-6 bg-white">Diagnóstico MK/Playhub</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {tasks.length === 0 ? (
