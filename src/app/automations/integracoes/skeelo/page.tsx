...
import { Play, Square, Pause, LogOut, CheckCircle2, XCircle, AlertTriangle, Clock, Loader2, Info, FileText, FileSpreadsheet, Trash2, ArrowLeft, Zap, Copy, BarChart2 } from "lucide-react";
...
export default function SkeeloSyncPage() {
  ...
  useEffect(() => {
    ...
    if (!storedToken || !storedDomain) {
      ...
      addLog("info", `Sessão iniciada p/ SKEELO no domínio: ${storedDomain}`);
    }
  }, [router]);
  ...
  const loadIdsIntoFila = (ids: string[]) => {
    ...
    // Filtra IDs válidos (apenas números para garantir que são contratos reais) e remove duplicados
  ...
  const processQueue = async () => {
    ...
    addLog("info", "Iniciando sincronização de contratos em lote...");
    ...
    for (let i = 0; i < tasks.length; i++) {
      ...
      try {
        const res = await fetch("/api/mk/skeelo", {
          method: "POST",
        ...
        if (res.ok && data.success) {
          updateTask(task.id, { status: "success", message: "Ativado/Sincronizado na base." });
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-[#414040] tracking-tight flex items-center gap-2">
                <Zap className="w-6 h-6 text-[#15864d]" />
                Sincronizador Skeelo
              </h1>
          ...
          <div className="md:col-span-1 space-y-6">
            ...
            <div className="bg-white p-5 rounded border border-gray-200/60 shadow-sm">
              ...
              <div className="pb-4">
                ...
                {/* Custom yellow/orange progress logic handled via background color inline for simplicity or override classes in tailwind */}
              ...
              {!isRunning ? (
                <div className="flex flex-col gap-2">
                  <Button 
                    ...
                    className="w-full bg-[#15864d] hover:bg-[#127040] text-black shadow-none font-semibold h-10 rounded"
                  >
                    ...
                    <Table className="w-full border-b-0">
                      <TableHeader className="bg-white sticky top-0 z-10 shadow-[0_2px_4px_rgba(0,0,0,0.02)]">
                        <TableRow className="hover:bg-transparent border-gray-200">
                          ...
                          <TableHead className="font-semibold text-[#414040] py-4 px-6 bg-white">Diagnóstico MK/Skeelo</TableHead>
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {tasks.length === 0 ? (
